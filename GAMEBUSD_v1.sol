...
contract GAMEBUSD is Context, Ownable {
    ...
    uint256 constant private collectStep = 3600 * 24 * 7;
    ...
    function collect() external {
        ...
        address msgSender = _msgSender();
        require(users[msgSender].lastTime > 0, "err: no deposit");
        require(users[msgSender].lastTime + collectStep < block.timestamp, "err: not in time");
    ...
    function getRewardsSinceLastDeposit(address addr) public view returns(uint256) {
        uint256 secondsPassed=min(collectStep, block.timestamp - users[addr].lastTime);
